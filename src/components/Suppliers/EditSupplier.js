...
export default function EditSupplier() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const r = await fetch(`http://localhost/ims/endpoints/suppliers/get_supplier.php?id=${id}`);
        ...
        console.error(err);
        setMessage("❌ Error fetching supplier");
      }
    })();
  }, [id]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const res = await fetch(`http://localhost/ims/endpoints/suppliers/update_supplier.php?id=${id}`, {
        method: "PUT",
  ...
  return (
    <div className="container-fluid">
      <div className="card-header d-flex justify-content-between align-items-center bg-warning">
                <h4 className="mb-0 m-4"> EDIT SUPPLIERS</h4>
